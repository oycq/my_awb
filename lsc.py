...
def load_lsc_gain_map(height: int, width: int) -> np.ndarray:
    ...
    # 插值到目标尺寸 (height, width, 3)
    gain_map_resized = cv2.resize(gain_map, (width, height), interpolation=cv2.INTER_CUBIC)
    
    return gain_map_resized.astype(np.float32)

def apply_lsc(img):
    # 假设img是浮点BGR图像；如果不是，需要调整
    lsc_gain_map = load_lsc_gain_map(img.shape[0], img.shape[1])
    img_corrected = img * lsc_gain_map
